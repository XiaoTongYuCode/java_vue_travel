...
export default {
  ...
  methods:{
    ...
    buy(){
      if (this.user.money/100 < this.scenicData.tickets){
        this.$message({
            type: 'warning',
            message: '余额不足，请充值'
          });
      ...
      this.$confirm('确认购买'+this.scenicData.name+'的门票吗？', '提示', {
          ...
          this.$axios.post(this.$httpUrl + "/order/add",{
            ...
            no: this.user.no,
            name: this.user.name,
            tickets: this.scenicData.tickets,
          }).then(res => res.data).then(res => {
            if (res.code === 200){
              this.$notify({
...
<template>
<div >
    <el-page-header style="height: 25px; align-items: center; margin-left: 10px; margin-top: 5px;" @back="goBack" content="景区列表">
    </el-page-header>
  <div style="width: 100%; display: flex; justify-content: center; margin-bottom: 50px;">
      <el-card class="box-card">
        ...
        <el-image :src="scenicData.img?scenicData.img:'https://cube.elemecdn.com/6/94/4d3ea53c084bad6931a56d5158a48jpeg.jpeg'" style="width: 100%;max-height: 500px;">
        ...
        <el-divider><i class="el-icon-office-building"></i>附 近 房 源</el-divider>
        <el-empty v-if="scenicData.hotel&&scenicData.hotel.length === 0" :image-size="150" description="暂无房源数据"></el-empty>
        <div v-for="(item, index) in scenicData.hotel" :key="index" class="hotel">
          <el-image style="width: 200px;" :src="item.img?item.img: 'https://img0.baidu.com/it/u=3369353589,1949555918&fm=253&fmt=auto&app=138'"></el-image>
        ...
        <div style="display: flex; justify-content:flex-end;">
          <el-button icon="el-icon-upload" size="small"
             @click="addShow=true;" v-if="user.roleId===0">上传房源</el-button>
          <el-button icon="el-icon-s-ticket" size="small" style="margin-left: 10px;"
                     type="warning" @click="buy()">购票：{{scenicData.tickets}}元</el-button>
          <el-button icon="el-icon-chat-dot-square" size="small" style="margin-left: 10px;"
                     type="primary" @click="post2()">评论</el-button>
        </div>
        <div v-for="(item2,i) in scenicData.comment" :key="i" >
          <el-divider v-if="i===0"></el-divider>
          <div style="width: 100%; min-height: 55px;display: flex; justify-content: space-between; align-items: center;">
            <div style="display: flex; align-items:center;">
              <el-avatar :size="30" shape="square" style="margin: 0 10px;">{{ item2.name ? item2.name.charAt(0) : item2.no.charAt(0)}}</el-avatar>
              <div >
