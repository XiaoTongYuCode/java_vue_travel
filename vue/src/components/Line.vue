...
export default {
  ...
  data() {
    return {
      ...
      addRules: {
        ...
        txt: [
          { required: true, message: '请完善路线', trigger: 'blur' }
        ],
      },
    }
  },
  methods: {
    ...
    delScenic(id){
      this.$confirm('此操作将删除此路线 确认删除？', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(( ) => {
...
<template>
  <div>
    <div style="display: flex;margin: 20px 0;display: flex;justify-content: center;">
      ...
      <el-button icon="el-icon-upload" @click="addShow=true;" type="success" v-if="user.roleId===0">上传路线</el-button>
    </div>
      ...
      <el-card class="box-card" v-for="(item,index) in scenicData" :key="index" :body-style="{'padding':'0'}">
        ...
        <div style="position: absolute; width: 100%; bottom: 5px;display: flex; justify-content: space-between;">
          ...
          <el-button style="margin-right: 10px;" size="small"
                     @click="buy(item)">订购 {{item.tickets}} 元</el-button>
        </div>
      </el-card>
    </div>
    ...
    <el-dialog title="上传路线" :visible.sync="addShow" center fullscreen>
      <div class="addDialog">
        <el-form ref="addForm" :model="addForm" :rules="addRules" style="width:80%;" label-position="right">
          ...
          <el-form-item label="路线详情" prop="txt">
